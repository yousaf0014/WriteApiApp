...
class UserDataController extends BaseController 
{
    ...
    public function cleanString($string){
         ...
         $cleanStr = preg_replace('/[^A-Za-z0-9@.\'";,.?\-$@%&!+=\n\r ]/', '', $string);
         return $cleanStr;
    }
    ...
    public function newartical(Request $request,Project $project, Artical $artical){
        ...
        $artical->reference_text = $this->cleanString($request->text);
        $artical->coherence = !empty($request->temp) ? $request->temp:9;
        $artical->creativity = !empty($request->k) ? $request->k:2;
        $artical->size =  700;
    ...
    public function saveArtical(Request $request,Project $project){
        $articals = array();
        $user = Auth::user();
        if(empty($request->type) || $request->type == 'others' ){
            $validator = Validator::make($request->all(),[
                'title'=>'required',
                'artical'=> "required"
            ]);
            ...
            $articalObj->coherence = !empty($request->coherence) ? $request->coherence:9;
            $articalObj->creativity = !empty($request->creativity) ? $request->creativity:2;
            ...
            $articalObj->title = $request->title;
            $articalObj->artical = '';
            $articalObj->type = 'gpt3';
            $articalObj->sample_paragraph = $request->artical;
            $articalObj->line1 = $request->line1;
            $articalObj->line2 = !empty($request->line2) ? $request->line2:'';
            $articalObj->line3 = !empty($request->line3) ? $request->line3:'';
            $articalObj->line4 = !empty($request->line4) ? $request->line4:'';
            $articalObj->line5 = !empty($request->line5) ? $request->line5:'';
            $articalObj->coherence = !empty($request->coherence) ? $request->coherence:9;
            $articalObj->creativity = !empty($request->creativity) ? $request->creativity:2;
            $articalObj->copies = !empty($request->copies) ? $request->copies:5;
            $articalObj->json = 0;
    ...
    public function getArtical(Request $request,Artical $artical){
    	...
    	if($artical->json == 1){
    		$artical->artical = json_decode($artical->artical);
        }
    ...
    public function getUsage(Request $request)
    {
        ...
        $userPlanObj->where('purchased_at','<',$date)->where('user_id', $user->id)->where('plan_id','!=',13)->update(array('expired'=>1));
        
        $date = date('Y-m-d 00:00:00',strtotime('-365 days'));
        $userPlanObj->where('purchased_at','<',$date)->where('user_id', $user->id)->where('plan_id',13)->update(array('expired'=>1));
    ...
    public function saveNewArtical(Request $request,Project $project){
        $user = Auth::user();
        $validator = Validator::make($request->all(),[
            'text'=>'required',
            'temp'=> "required",
